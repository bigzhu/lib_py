...
def saveAndGetOauth(oauth_info):
    ...
    pg.insertOrUpdate('oauth_info', where=where, vars=oauth_info, **oauth_info)

    return pg.select('oauth_info', where=where, vars=oauth_info)
...
class QQAuth2Minix(OAuth2Mixin):
    ...
    def get_authenticated_user(self, redirect_uri, client_id, client_secret,
        ...
        response = yield gen.Task(http.fetch, self._oauth_request_token_url(**args), validate_cert=False, connect_timeout=100, request_timeout=100)
    ...
    def qq_request(self, path, method, open_id, token, client_id, callback, **args):
        ...
        http = self.get_auth_http_client()
        if "POST" == method:
            response = yield gen.Task(http.fetch, url, method=method, body=urllib.urlencode(params), validate_cert=False, request_timeout=100)
            self._on_qq_request(callback, response)
        else:
            url = url_concat(url, params)
            response = yield gen.Task(http.fetch, url, validate_cert=False, request_timeout=100)
            self._on_qq_request(callback, response)

    def _on_qq_request(self, callback, response):
        if response.error:
            logging.warning("Error response %s fetching %s header %s", response.error,
